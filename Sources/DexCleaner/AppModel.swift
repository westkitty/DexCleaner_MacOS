...
final class AppModel: ObservableObject {
    ...
    func selectVisibleCandidates() {
        let visibleIDs = Set(allCleanableItems.map(\.id))
        ...
        invalidatePreview()
        statusText = "Selected \(visibleIDs.count) verified Safe candidates. Preview is required before cleanup."
    }
    ...
    func toggleLaunchAtLogin() {
        ...
        do {
            ...
            statusText = launchAtLoginEnabled ? "Launch at login enabled." : "Launch at login disabled."
        } catch {
            launchAtLoginEnabled = SMAppService.mainApp.status == .enabled
            statusText = "Launch at login change failed: \(error.localizedDescription)"
        }
    }
